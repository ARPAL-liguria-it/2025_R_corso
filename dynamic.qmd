...
In questo capitolo imparerai a realizzare report personalizzati e flessibili utilizzando i parametri in documenti **Quarto**. Apprenderai a creare template riutilizzabili per analisi su dati diversi e a produrre automaticamente più report con script R o da linea di comando, migliorando l'efficienza, l'automazione e la riproducibilità del flusso di lavoro.
...
In questo caso abbiamo definito solo un parametro: il nome del comune.
...
Utilizzando i parametri, è possibile modificare dinamicamente anche il testo del report. Ad esempio, possiamo creare un'introduzione che si adatta ai parametri definiti, come mostrato qui:
...
Combiniamo tutti gli elementi sopra per creare un esempio completo di report parametrizzato in **Quarto**.
...
Salviamo il report nel file `report_dinamico.qmd`.

## Creazione di report multipli

Per utilizzare `report_dinamico.qmd` come template per la creazione di report personalizzati abbiamo due opzioni:
...
- quali sono le parti fisse del template?
