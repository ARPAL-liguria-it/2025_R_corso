...
Su vettori e matrici numeriche è possibile eseguire operazioni di algebra lineare, ad esempio il prodotto matriciale con `%*%` o la risoluzione di sistemi lineari tramite `solve()`. Nella pratica, tuttavia, queste operazioni sono raramente eseguite manualmente, perché funzioni e pacchetti dedicati integrano già routine efficienti per la maggior parte degli algoritmi di analisi.
