...
Si può verificare se un elemento è di tipo `NA` con la funzione `is.na()`.
 ...
 ########### NON MI PIACE ############ cambiare esempio
Talvolta, R può restituire un elemento di tipo `NULL` estraendo un elemento che non esiste da un qualche oggetto. Più comunemente, è possibile assegnare il valore `NULL` a un elemento per eliminarlo:
...
Con vettori possiamo iniziare a fare qualche conto con funzioni quali `max()`, `min()`, `sum()`, `mean()`, `median()`, `quantile()`, `which.min()` e `which.max()` e molte altre.
