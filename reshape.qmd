# Cambiare la forma ai dati {#sec-reshape}
...
-   *wide*: con il termine largo si intende dati organizzati con un'entità osservata per riga e una misura per colonna. Vediamo un esempio:
...
-   *long*: con il termine lungo si intende dati organizzati con una misura per riga. La tipologia della misura è archiviata all'interno di un'apposita variabile, come mostrato nell'esempio:
...
la funzione, si è accorta che compaiono misure ripetute e, non sapendo cosa vogliamo farci, si limita a contare il numero di misure per ogni unità e ogni tipologia di misura.
...
```{r}
#| echo: false
pm_genova <- fread("data/pm_genova_int1.csv.gz")
```
