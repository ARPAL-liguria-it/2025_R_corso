...
A partire dal @sec-basics si è organizzato il codice in R utilizzando script e aggiungendo commenti per chiarire le parti più complesse. Tuttavia, in progetti di analisi dati complessi, è necessario integrare anche documentazione testuale per collegare il codice ai risultati. In questi casi, **Quarto** si rivela uno strumento utile.
...
1. si segue **File > New File > Quarto Document**;
2. si inserisce il titolo e l'autore nel pannello di creazione;
3. si sceglie il formato di output (default HTML, ma puoi selezionare PDF, Word, ecc.);
...
All'interno di un documento **Quarto** il testo è inserito semplicemente scrivendo al di sotto dell'intestazione e utilizzando la tipica sintassi **markdown**, ormai sempre più diffusa. Ecco un esempio:
