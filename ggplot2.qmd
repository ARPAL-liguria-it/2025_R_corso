...
In questo capitolo imparerai a creare grafici professionali e personalizzati con il pacchetto `ggplot2`, basato sulla *grammar of graphics*. Apprenderai a sovrapporre più livelli per arricchire le visualizzazioni, personalizzare assi, colori, legende e temi.
...
Vediamo un esempio per la creazione di un semplice grafico di dispersione a partire da un dataset sintetico:
...
Vediamo un esempio in cui personalizziamo l'asse temporale e l'aspetto della legenda:
...
Utilizzando il dataset `pm_genova` importato e modificato nelle sezioni precedenti, rappresentare la distribuzione dei valori medi giornalieri di PM~10~ per i diversi comuni. Raffrontare visivamente i valori medi giornalieri con il limite di $50\, \mu g/m^3$.
